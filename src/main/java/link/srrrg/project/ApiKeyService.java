...
public class ApiKeyService {
	...
	public ApiKeyService(ProjectApiKeyRepository keys, ProjectMemberRepository members, ProjectRepository projects,
		...
		this.keys = keys; this.members = members; this.projects = projects; this.users = users; this.random = random;
	}

	@Transactional(readOnly = true)
	public List<ProjectApiKey> list(Long userId, Long projectId) { requireOwner(userId, projectId); return keys.findByProjectIdOrderByCreatedAtDesc(projectId); }
	@Transactional
	public CreatedKey create(Long userId, Long projectId, String name, Set<ApiKeyScope> scopes, Instant expiresAt) {
		...
		if (name == null || name.isBlank() || name.trim().length() > 100) throw new IllegalArgumentException("API key 이름은 1~100자로 입력하세요.");
		if (scopes == null || scopes.isEmpty()) throw new IllegalArgumentException("API key scope를 하나 이상 선택하세요.");
		if (expiresAt != null && !expiresAt.isAfter(Instant.now())) throw new IllegalArgumentException("만료 시각은 미래여야 합니다.");
		String prefix = random.generate(CHARS, 8);
		String raw = "srrrg_pk_" + prefix + "_" + random.generate(CHARS, 43);
		ProjectApiKey key = ProjectApiKey.create(project(projectId), name.trim(), prefix, hash(raw), user(userId), scopes, expiresAt);
		keys.save(key);
	...
	public void revoke(Long userId, Long projectId, Long keyId) {
		...
		if (!key.getProject().getId().equals(projectId)) throw new IllegalArgumentException("API key를 찾을 수 없습니다.");
		key.revoke();
	}
	@Transactional
