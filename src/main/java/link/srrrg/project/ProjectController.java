...
public class ProjectController {
	...
	@PostMapping("/projects") public ResponseEntity<ProjectResponse> create(@AuthenticationPrincipal SrrrgPrincipal p, @Valid @RequestBody CreateProjectRequest request) { Project project = projects.create(p.userId(), request.name(), request.subdomain()); return ResponseEntity.status(HttpStatus.CREATED).body(ProjectResponse.from(project, ProjectRole.OWNER)); }
	...
	@PatchMapping("/projects/{projectId}") public ProjectResponse rename(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId, @Valid @RequestBody RenameProjectRequest request) { return ProjectResponse.from(projects.rename(p.userId(), projectId, request.name()), ProjectRole.OWNER); }
	@DeleteMapping("/projects/{projectId}") public ResponseEntity<Void> deleteProject(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId) { projects.delete(p.userId(), projectId); return ResponseEntity.noContent().build(); }
	@GetMapping("/projects/{projectId}/overview") public ProjectOverviewResponse overview(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId) {
		List<CampaignResponse> campaignSummaries = campaigns.list(p.userId(), projectId, null, 100).stream().map(CampaignResponse::from).toList();
	...
	@GetMapping("/projects/{projectId}/subdomain") public SubdomainResponse subdomain(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId) { return SubdomainResponse.from(projects.projectDomain(p.userId(), projectId)); }
	@PutMapping("/projects/{projectId}/subdomain") public SubdomainResponse claimSubdomain(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId, @Valid @RequestBody ChangeSubdomainRequest request) { return SubdomainResponse.from(projects.claimSubdomain(p.userId(), projectId, request.subdomain())); }
	@PatchMapping("/projects/{projectId}/subdomain/activation") public SubdomainResponse activateSubdomain(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId, @Valid @RequestBody SubdomainActivationRequest request) { return SubdomainResponse.from(projects.setSubdomainEnabled(p.userId(), projectId, request.enabled())); }
	@DeleteMapping("/projects/{projectId}/subdomain") public SubdomainResponse releaseSubdomain(@AuthenticationPrincipal SrrrgPrincipal p, @PathVariable Long projectId) { return SubdomainResponse.from(projects.releaseSubdomain(p.userId(), projectId)); }
	...
	public record CreateProjectRequest(@NotBlank @Size(max = 100) String name, @Size(min = 3, max = 63) String subdomain) { }
	...
	public record ProjectResponse(Long id, String name, String subdomain, boolean subdomainEnabled, ProjectRole role) {
		static ProjectResponse from(ProjectMember member) { return from(member.getProject(), member.getRole()); }
		static ProjectResponse from(Project project, ProjectRole role) { return new ProjectResponse(project.getId(), project.getName(), project.getSubdomain(), project.isSubdomainEnabled(), role); }
	}
	public record ProjectOverviewResponse(List<ProjectLinkResponse> standaloneLinks, List<CampaignResponse> campaigns) { }
	public record SubdomainResponse(String subdomain, boolean enabled) { static SubdomainResponse from(Project project) { return new SubdomainResponse(project.getSubdomain(), project.isSubdomainEnabled()); } }
	public record ProjectLinkResponse(String code, String subdomain, String name, String originalUrl, Instant expiresAt, Instant createdAt) { static ProjectLinkResponse from(Link link) { return new ProjectLinkResponse(link.getCode(), link.getSubdomain(), link.getName(), link.getOriginalUrl(), link.getExpiresAt(), link.getCreatedAt()); } }
