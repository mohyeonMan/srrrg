package link.srrrg.campaign.link.csv.repository;
...
public interface CampaignImportRepository extends JpaRepository<CampaignImport, Long> {
	...
	@Query("""
			update CampaignImport i
			   set i.status = link.srrrg.campaign.link.csv.model.ImportStatus.CANCELLED,
			       i.completedAt = CURRENT_TIMESTAMP,
			       i.leaseOwner = null,
			       i.leaseExpiresAt = null
			 where i.campaign.id = :campaignId
			   and i.status in (link.srrrg.campaign.link.csv.model.ImportStatus.PENDING, link.srrrg.campaign.link.csv.model.ImportStatus.PROCESSING)
			""")
	/**
