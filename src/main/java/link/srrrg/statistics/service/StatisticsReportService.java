...
import link.srrrg.statistics.repository.StatisticsQueryRepository;
...
import link.srrrg.statistics.repository.StatisticsQueryRepository.Dimension;
...
import link.srrrg.statistics.dto.StatisticsResponse.LifetimeLinkSummary;
...
import link.srrrg.statistics.dto.StatisticsResponse.Scope;
import link.srrrg.statistics.dto.StatisticsResponse.Summary;
 ...
 *
 * <p>인가는 하지 않는다. 상위 애플리케이션 경계인 {@link StatisticsService}가 접근 권한과 조회 대상을
 * 확정한 뒤 식별자를 넘긴다는 전제다. 컨트롤러나 다른 호출자가 이 서비스를 직접 사용하면 그 경계를
 * 우회하므로, 새 통계 진입점도 반드시 {@code StatisticsService}에 유스케이스를 먼저 추가한다.</p>
 *
...
public class StatisticsReportService {
	...
	@Autowired
	public StatisticsReportService(StatisticsQueryRepository queries) {
		this(queries, Clock.system(ZONE));
	}

	StatisticsReportService(StatisticsQueryRepository queries, Clock clock) {
