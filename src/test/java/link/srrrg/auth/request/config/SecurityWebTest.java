package link.srrrg.auth.request.config;

import link.srrrg.auth.login.adapter.DatabaseAuthorizationRequestRepository;
import link.srrrg.auth.login.controller.LoginController;
import link.srrrg.auth.login.handler.OAuthLoginFailureHandler;
import link.srrrg.auth.login.handler.OAuthLoginSuccessHandler;
import link.srrrg.auth.login.service.ProviderOAuth2UserService;
import link.srrrg.auth.login.service.ProviderOidcUserService;
import link.srrrg.auth.request.config.SecurityConfiguration;
import link.srrrg.auth.request.filter.CsrfCookieFilter;
import link.srrrg.auth.request.service.ApiKeyRequestAuthorizer;
import link.srrrg.auth.session.controller.AuthController;
import link.srrrg.auth.session.service.JwtService;
import link.srrrg.auth.session.service.RefreshTokenService;
import link.srrrg.auth.session.service.WebSessionService;
import link.srrrg.auth.session.service.WebTokenCookies;
...
import link.srrrg.web.page.controller.HomeController;
import link.srrrg.campaign.service.CampaignService;
import link.srrrg.common.ratelimit.service.RateLimitService;
import link.srrrg.identity.account.controller.AccountController;
...
import link.srrrg.link.access.model.ClientRequestInfo;
import link.srrrg.link.access.service.ClientRequestInfoResolver;
import link.srrrg.link.management.controller.LinkController;
import link.srrrg.link.management.service.LinkManagementService;
import link.srrrg.link.management.controller.ProjectLinkController;
...
import link.srrrg.project.invitation.controller.InvitationPageController;
import link.srrrg.project.invitation.service.ProjectInvitationService;
import link.srrrg.project.controller.ProjectController;
import link.srrrg.project.subdomain.controller.ProjectSubdomainController;
...
@WebMvcTest(controllers = {HomeController.class, LoginController.class, LinkController.class, ProjectLinkController.class,
		...
		InvitationPageController.class, ProjectController.class, ProjectSubdomainController.class, AccountController.class})
...
class SecurityWebTest {
	...
	void returnsCursorWhenMoreProjectLinksExist() throws Exception {
		link.srrrg.link.model.Link first = org.mockito.Mockito.mock(link.srrrg.link.model.Link.class);
		link.srrrg.link.model.Link second = org.mockito.Mockito.mock(link.srrrg.link.model.Link.class);
	...
	void returnsProjectPlatformDomainToJwtMember() throws Exception {
		...
		when(projectSubdomainService.get(1L, 7L)).thenReturn(project);
	...
	void changesProjectPlatformDomainWithJwtAndCsrf() throws Exception {
		...
		when(projectSubdomainService.claim(1L, 7L, "renamed")).thenReturn(project);
		jakarta.servlet.http.Cookie jwt = new jakarta.servlet.http.Cookie("srrrg_access", "access-token");
		MvcResult page = mockMvc.perform(get("/login")).andExpect(status().isOk()).andReturn();
		jakarta.servlet.http.Cookie csrf = page.getResponse().getCookie("XSRF-TOKEN");
	...
	void createsProjectLinkWithApiKeyWithoutJwtFilter() throws Exception {
		...
		link.srrrg.link.model.Link link = org.mockito.Mockito.mock(link.srrrg.link.model.Link.class);
