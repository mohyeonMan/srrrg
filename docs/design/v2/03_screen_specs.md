...
```text
URL을 입력하면 단축 링크와 관리용 secret key를 바로 발급합니다. 신뢰되지 않은 링크는 이동 전 원본 주소를 먼저 보여줍니다.
```
...
현재 컨셉인 `신뢰할 수 있는 링크인가요?`는 유지한다.

### 6.1 목적

사용자가 자동 이동되지 않는 링크의 원본 주소를 확인하고, 이동 여부를 직접 결정한다.

### 6.2 화면 우선순위

1. 확인이 필요한 상황임을 설명
2. 원본 도메인 표시
3. 전체 원본 URL 표시 또는 펼치기
4. 단축 URL 표시
5. 이동/돌아가기 선택
...
```text
신뢰할 수 있는 링크인가요?
이 단축 링크는 이동 전 확인이 필요합니다. 원본 주소를 확인한 뒤 이동하세요.
...
```text
이 단축 링크는 자동 이동 대상이 아닙니다. 원본 주소를 확인한 뒤 이동하세요.
```
...
도메인을 URL보다 크고 진하게 보여준다.

```text
example.com
```

전체 URL은 도메인 아래에 상대적으로 작게 둔다.

```text
https://example.com/path?query=value
```

사용자는 긴 URL 전체보다 도메인으로 먼저 위험을 판단한다.

### 6.6 돌아가기 동작

권장 순서:

1. `document.referrer`가 있으면 `history.back()`
2. referrer가 없으면 `/`로 이동

서버 렌더링만으로 구현할 경우 secondary link는 홈(`/`)으로 둔다.
...
| 기타 | `링크를 열 수 없습니다` | `요청한 링크를 처리하는 중 문제가 발생했습니다.` |
