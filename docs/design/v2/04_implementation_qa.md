...
6. 안전 판정 링크는 별도 화면 없이 즉시 `302`로 이동한다.
7. 위협 및 검증 불가 화면에 홈과 재시도 action을 구분해 제공한다.
...
- 안전 판정은 즉시 `302`로 이동한다.
- 위협 판정은 `403`이며 원본 URL 이동 액션이 없다.
- 위협 판정에는 Google 출처와 판정 한계를 표시한다.
- 검증 불가는 `503`이며 재시도만 제공한다.
