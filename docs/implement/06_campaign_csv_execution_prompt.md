...
- link_utm_values가 링크와 템플릿 필드를 N:N 연결하면서 최종 값을 저장한다.
- 필드 추가는 해당 템플릿을 사용하는 캠페인의 새 양식과 새 링크에 반영한다.
- 필드 삭제는 soft delete이며 기존 링크 값, 리다이렉트와 과거 통계용 관계를 유지한다.
- 필드 이름 변경은 기존 필드 soft delete 후 새 필드 생성으로 처리한다.
- 캠페인 기본값 변경은 새 링크에만 적용한다.
- 위 항목은 UTM 기본값에만 적용한다. 캠페인 기본 목적지는 자체 목적지가 없는 링크가 리다이렉트될 때 동적으로 사용한다.
