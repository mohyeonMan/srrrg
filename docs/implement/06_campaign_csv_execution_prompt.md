...
- 링크·기본값·import의 UTM은 `field_name` 관계로 저장하고, 접근 당시 최종 UTM만 이벤트 JSONB에 저장한다.
- 캠페인은 현재 utm_template_id와 캠페인별 필드 기본값을 가진다.
- 링크는 생성 당시 utm_template_id를 snapshot으로 저장한다.
- link_utm_values는 링크에 직접 지정한 값을 필드 이름으로 저장한다.
- 필드 추가는 해당 템플릿을 사용하는 캠페인의 양식·리다이렉트·통계에 즉시 반영하고, 기본값을 설정하면 값이 없는 기존 링크도 동적으로 상속한다.
- 필드 삭제는 soft delete하며 값은 보존하되 현재 리다이렉트와 통계에서 숨긴다. 같은 이름을 다시 추가하면 보존 값을 재사용한다.
