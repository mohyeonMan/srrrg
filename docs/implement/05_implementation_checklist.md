...
- [x] 캠페인 기본 목적지 동적 fallback·UTM 기본값 복사
...
> 캠페인 기본 목적지는 링크에 복사하지 않는다. 자체 목적지가 없는 링크가 리다이렉트될 때 현재 캠페인 기본 목적지를 사용하며, 둘 다 없으면 `410 Gone`을 반환한다. UTM 기본값은 생성 시 링크에 복사한다.
