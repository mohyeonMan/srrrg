...
- 프론트엔드, 회원, 로그인, 팀 기능
- 커스텀 alias 및 서브도메인
- 상세 클릭 통계와 별도 클릭 이벤트 테이블
- Redis, 캐시, 메시지 큐, 비동기 클릭 집계
...
클릭 수는 별도 이벤트를 저장하지 않고 `click_count = click_count + 1` 형태의 원자적 update query로 증가시킨다. 요청마다 엔티티를 조회하고 저장하는 방식보다 동시 요청에서 값 유실을 피하기 쉽고, v1 범위도 늘리지 않는다.
...
클릭 수 증가 실패 시 리다이렉트까지 실패시킬지는 구현 시 테스트로 명확히 한다. v1 기본 방향은 저장 일관성을 위해 같은 요청 흐름에서 처리하는 것이다.
...
완료 기준: 정상 링크는 302와 정확한 `Location`을 반환하고, 만료·삭제·없는 링크 및 동시 클릭 증가를 테스트한다.
