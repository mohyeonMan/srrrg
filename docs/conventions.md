...
- Controller는 Repository에 직접 의존하거나 호출하지 않는다. 단순 조회도 application 또는 유스케이스 Service를 거친다.
- Service는 유스케이스, 권한, 조회와 트랜잭션 경계를 조율한다.
- Repository는 데이터 접근을 담당하고 업무 흐름을 소유하지 않는다.
- API 응답에 JPA entity를 직접 반환하지 않는다.
- 다른 도메인의 책임은 해당 도메인 패키지와 소유 서비스에 둔다.

Controller의 Repository 호출을 감추기만 하는 의미 없는 위임 메서드는 만들지 않는다. Service 메서드는
Repository의 기술적 연산이 아니라 호출자가 수행하려는 유스케이스와 접근 경계를 이름과 계약으로 표현한다.
엔드포인트나 단일 기능마다 Service를 기계적으로 만들지 않고, 같은 업무 규칙·트랜잭션·접근 정책을
공유하는 유스케이스를 응집된 Service로 구성한다. 변경 이유·생명주기·실패 정책이 달라지면 분리한다.
이를 위해 구현체 하나뿐인 인터페이스나 factory를 추가하지 않는다.
